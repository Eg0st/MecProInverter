...
void Power_Up::enter(State_Machine* pOwner, Interface* interface)
{
  ...
  interface->oneWire=new OneWire(ONE_WIRE_BUS);
...
{
  PWM_GEN::init(interface->driver,interface->motor);
...
void Run::Execute(State_Machine* pOwner, Interface* interface)
{
  if(!digitalRead(SW1))
  {
    ...
    PWM_GEN::PWM_Generation(interface->driver,interface->motor,new_target_value);

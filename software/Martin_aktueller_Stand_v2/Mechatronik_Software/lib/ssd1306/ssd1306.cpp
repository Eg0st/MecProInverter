...
static uint8_t ssd1306_WriteCommand(TwoWire* Wire, uint8_t byte) {
	Wire->beginTransmission((uint8_t) SSD1306_I2C_ADDR);
...
uint8_t ssd1306_Init(TwoWire*handle) {
...
void ssd1306_UpdateScreen(TwoWire* Wire)
{
    ...
    //  * 64px   ==  8 pages
    //  * 128px  ==  16 pages
    for(uint8_t u = 0; u < SSD1306_HEIGHT/8; u++)
    {
        ssd1306_WriteCommand(Wire,0xB0 + u); // Set the current RAM page address.
        ssd1306_WriteCommand(Wire,0x00);
        ssd1306_WriteCommand(Wire,0x10);
        ...
        for(uint8_t i=0;i<SSD1306_WIDTH;i++)
        {
            if (bytesOut >= 32) {
                ...
                Wire->write(SSD1306_Buffer[i+(u*SSD1306_WIDTH)]);
                bytesOut++;
        } 
    }
    Wire->endTransmission();
